...
void build_zc_tx()
{
    // First half = ZC sequence/cyclic prefix
    for(int i = 0; i < NSAMP; i++)
        tx_zc[i] = zc[i];

    // Second half (ZC sequence)
    for(int i = 0; i < NSAMP; i++)
        tx_zc[i + NSAMP] = zc[i];

    L_zc = 2 * NSAMP;

    // both halves are Zc squences, tx [ zc | zc ] with fisrt copy being cyclic prefix
}
 ...
 *      apply DFT butterfly with twiddle factor w = e^(-j*2*pi/len).
 ***************************************************************************/
void fft_complex(float complex *x, int N)
{
    // Bit-reversal permutation
    int j = 0;
    for(int i = 1; i < N; i++) // begin from 1 since bitreversal of 0 is 0
    {
        int bit = N >> 1;
        while(j & bit) 
        { 
            j ^= bit;  // clear the bit and move to the next lower bit, xor-equals (toggle) the bit 
            bit >>= 1;  // shift to the next lower bit, bit is used to find the highest set bit in i
        }
        j |= bit; // bits in j are set to the next bit position, this is the bit-reversal index for i
        if(i < j) // swap x[i] and x[j] to put them in bit-reversed order
        {
    ...
    for(int len = 2; len <= N; len <<= 1)
    {
        ...
        float complex wlen = cosf(angle) + I * sinf(angle);  // Base twiddle factor

        for(int i = 0; i < N; i += len)
        {
            float complex w = 1.0f + 0.0f * I;  // Running twiddle factor
            for(int j = 0; j < len/2; j++)
            {
                float complex u = x[i+j]; // even index (top of butterfly)
                float complex v = x[i+j+len/2] * w; // odd index (bottom of butterfly) multiplied by twiddle factor
                x[i+j]         = u + v;   // Butterfly sum
                x[i+j+len/2]   = u - v;   // Butterfly difference
                w *= wlen;                 // Advance twiddle factor
            }
        }
    }
}
 ...
 ***************************************************************************/
void fftshift_1d(float complex *x, int N)  // N must be even
{
    int half = N / 2; // Halfway point for swapping
                            ...
                            float complex *out, int N)
{
    ...
    for(int i = 0; i < N; i++) { RX[i] = rx_in[i]; 
        TX[i] = tx_in[i]; }
